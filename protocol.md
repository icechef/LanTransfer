...
- 手机端选定源目录后，将文件以 `relPath = "同步_<设备名>/<相对路径>"` 发送到电脑端 TCP 端口；电脑端按现有 `relPath` 逻辑落到 `接收目录/同步_<设备名>/<相对路径>`。
- 同步为**单向增量镜像**（手机 → 电脑）：手机端本地记录已同步文件的 `relPath + mtime + size`，仅发送新增/变更的文件；不做删除传播（安全起见）。
- 触发时机：设备空闲（充电中且熄屏）、手动「立即同步」、进入同步页时。
